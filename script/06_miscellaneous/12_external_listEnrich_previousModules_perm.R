...
# prepare input file for Voinague modules
...
# prepare input file for Parikshak modules

gmt=data.table::fread("D:/datasets/Geschwind_modules/41586_2016_BFnature20612_MOESM114_ESM_sheet1.txt")
gmt=gmt[,c(38,1)]
colnames(gmt)=c("term","geneid")
gmt$term=paste("Cortex_M",gmt$term,sep = "")

# prepare input file for Gandal modules
gmt=data.table::fread("D:/datasets/Geschwind_modules/DER-16_Disorder_Gene_Modules.csv",select = c(2,1))
colnames(gmt)=c("term","geneid")
...
for(i in 2:length(unique(modules))) {
  ...
  for (j in unique(gmt$term)){
    genesets=unique(gmt$geneid[gmt$term==j])
...
ggplot(permTable, aes(x=Module1,y=category, label=text)) +
  ...
  scale_fill_gradient2(low = "blue", mid = "white", high = "red","-log10FDR\n") + 
