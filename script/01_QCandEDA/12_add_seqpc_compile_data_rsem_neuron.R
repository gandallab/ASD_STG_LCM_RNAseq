...
genes_to_keep = filterByExpr(DGEList(counts))
table(genes_to_keep)
counts10=counts[genes_to_keep,]
rownames(counts10)=sapply(rownames(counts10),function(x) str_split_fixed(x,"\\.",Inf)[1,1])
annot=read.delim("D:/references/gencode.v19.gene.name.txt")
counts10$genenames=annot$genename[match(rownames(counts10),annot$geneid)]
counts1=counts[rowSums(counts>0) > 0.8*ncol(counts),]
table(datMeta$Sample == colnames(counts1))
counts2=round(as.matrix(counts1))

se_neuron=SummarizedExperiment(assays = list(counts=counts2),colData=datMeta)
save(file = "working_data/summarizedExperiment/newrsem/se_neuron.RData",se_neuron)
