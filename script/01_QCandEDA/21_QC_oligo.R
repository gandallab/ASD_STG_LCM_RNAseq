...
load("working_data/summarizedExperiment/fc_2pass/se_oligo.RData")
...
#genes_to_keep = filterByExpr(calcNormFactors(DGEList(assays(se_oligo)$counts)),group = colData(se_oligo)$Diagnosis)
...
km=kmeans(Z.C,2)
table(names(km$cluster) == colnames(se_oligo))
...
save(file="working_data/summarizedExperiment/fc_2pass/se_oligo_CPM_outlierRemoved.RData", se_oligo)
