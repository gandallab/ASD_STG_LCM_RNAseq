...
distunparsed = read.table("../3mOrganoid.exprGenes.fulldata/YourData.output",sep="}")$V1;
...
datTraits2=datTraits1[datTraits1$Lab=="Iakoucheva",]
dist$Stage=dist$Stage[,rownames(datTraits2)]
dist$Region=dist$Region[,rownames(datTraits2)]
cat('Plotting Stage...\n');
...
labeledHeatmap(Matrix = dist$Stage,
               xLabels = datTraits$Group, #with(datTraits2,paste(Group,Lab)),
