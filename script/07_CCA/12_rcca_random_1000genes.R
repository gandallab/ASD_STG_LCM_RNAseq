...
for (i in cca_files) {
  ...
  ind = sample(1:ncol(expr_x),1000,replace = F)
  expr_x1=expr_x[,ind]
...
save(final_output,file = "working_data/cca/variable_cor_random.RData")
final_output1=final_output %>% filter(abs(correlation) > 0.7)
#final_output1=final_output %>% filter(grepl("^14:",cluster))
write.table(final_output1,file = "working_data/cca/top07correlation_random.txt",quote = F,sep = "\t",row.names = F)

rm(list = ls());gc()
target=read.delim("working_data/cca/top07correlation.txt")
bg=read.delim("working_data/cca/top07correlation_random.txt")
target1=target[!target$cluster %in% bg$cluster,]
