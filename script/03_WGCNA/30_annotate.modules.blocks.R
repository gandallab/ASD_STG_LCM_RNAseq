...
# GWAS enrichment
gwas=read.delim("working_data/wgcna/blocks.fc/block.gwas.enrich.txt")
...
for(i in 2:ncol(MEs$eigengenes)) {
  ...
  # neruon MEs
  me1=ME.exprNeuron.moduleBlocks$eigengenes[,i]
  
  cairo_pdf(file=paste0("working_data//figures/moduleAnnot.blocks/", moduleNumber, moduleColor, ".pdf"), width=20, height=16,family = "Arial")
  
  dat2=rbind(modTrait[modTrait$moduleNumber == moduleNumber , c(1:8)],modTrait.exprNeuron.moduleBlocks[modTrait.exprNeuron.moduleBlocks$moduleNumber == moduleNumber , ])
  dat2$Group=c("Blocks","Neuron")
...
#    scale_color_manual(values = c("black","red","blue"))+
  ...
  #Gene ontology  
  go = read.delim(paste("working_data/wgcna/blocks.fc/go/go.M",moduleNumber,"_",moduleColor,".txt",sep = ""),header = T,sep = "\t")
  go=go[complete.cases(go),]
  #go=go[go$overlap.size > 1,]
  #go=go[go$NES > 0,]
  go=go[1:min(15,nrow(go)),]
  if(nrow(go)==0){
    df <- data.frame()
  ...
  # gwas
  currentgwas=gwas[gwas$SET == moduleColor,]
  currentgwas$fdr=p.adjust(currentgwas$Enrichment_p,method = "fdr")
  g6=ggplot(currentgwas, aes(x=reorder(GWAS, -log10(Enrichment_p)), y=-log10(fdr))) + geom_bar(stat="identity", fill="red") + 
    coord_flip() + xlab("") + 
  ...
  grid.arrange(grobs=list(g1,g2,g3,g4,g5,g6), layout_matrix=rbind(c(1,2,2,5,5,6,6,6),c(3,3,3,3,4,4,4,4)),
