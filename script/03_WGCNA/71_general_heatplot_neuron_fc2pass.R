...
to.keep=paste("M",seq(1,18),sep = "")
#to.keep=paste("M",c(5,6,11,16,17),sep = "")
...
heatdata=read.delim("working_data/wgcna/neuron.fc2pass/neuron_fc2pass_ds2mod100/modTrait.neuron.fc2pass.txt")
...
heatdata$moduleNumber=paste("M",heatdata$moduleNumber,sep = "")
heatdata=heatdata[heatdata$moduleNumber %in% to.keep,]
heatdata=heatdata[grepl("Ex|In",heatdata$CellType),]
...
plt_hmap2 <- ggplot(heatmap_data, 
  ...
  scale_x_continuous(expand = c(0,0),
                     ...
                     breaks = gene_pos_table$x_center,
                     sec.axis = dup_axis())+
  scale_y_reverse(breaks = sample_pos_table$y_center, 
...
# heatmap3
load("working_data/listEnrich/permutTable.1000times.neuron_fc2passModule.newhouseList.RData")
heatdata=permTable
...
heatdata=read.delim("working_data/wgcna/neuron.fc2pass/neuron_fc2pass_ds2mod100/neuron.fc2pass.gwas.enrich.txt")
...
heatdata=heatdata[heatdata$moduleNumber %in% to.keep,]
heatdata=heatdata[,c("fdr" , "text"  , "moduleNumber")]
heatdata$GWAS = "ASD_GWAS"
heatdata=rbind(heatdata,data.frame(GWAS = "ASD_GWAS", fdr =1, text = "", moduleNumber="M4"))
...
cairo_pdf("working_data/final/panels/neuron_fc2pass.module.heatplot.raw.all.pdf",width = 7,height = 10,family = "Arial")
#plot_grid(plt_dendr,plt_hmap1, plt_hmap4, plt_hmap2, plt_hmap3,  
#          ncol = 1,align = "v",rel_heights = c(3.5,2.2,2.2,8,11))
