...
# GWAS enrichment
gwas=read.delim("working_data/wgcna/neuron.fc/neuron.gwas.enrich.txt")
...
for(i in 2:ncol(MEs$eigengenes)) {
  ...
  cairo_pdf(file=paste0("working_data//figures/moduleAnnot.neuron/", moduleNumber, moduleColor, ".pdf"), width=20, height=16,family = "Arial")
  ...
  #Gene ontology  
  go = read.delim(paste("working_data/wgcna/neuron.fc/go.manualRemove/go.M",moduleNumber,"_",moduleColor,".txt",sep = ""),header = T,sep = "\t")
