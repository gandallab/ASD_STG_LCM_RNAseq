...
graph2ppt(file="working_data/figures/oligo.pptx",width=8.5,height=6.5,append=TRUE)
...
print(p)
graph2ppt(file="working_data/figures/oligo.pptx",width=9,height=2.7,append=TRUE)
write.table(modTrait,file = "working_data/wgcna/oligo.fc//modTrait.oligo.fc.txt",
            quote = F,
            sep="\t",
            row.names = F)
...
for(i in 2:length(unique(modules))){
  ...
  moduleNumber = as.numeric(gsub("ME", "", colnames(MEs$eigengenes)[i]))
  moduleColor = labels2colors(moduleNumber)
  if (!moduleColor %in% modTrait$Module[modTrait$fdr < 0.06]){next}
  ...
  write.table(go,file=paste("working_data/wgcna/oligo.fc/go/go.M",moduleNumber,"_",moduleColor,".txt",sep=""),quote=F,sep="\t",row.names=F)
}
