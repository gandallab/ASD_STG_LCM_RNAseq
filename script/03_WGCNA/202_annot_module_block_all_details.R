...
load("working_data/listEnrich/permutTable.1000times.block_fc2passModule.Gandal.RData")
gandal=permTable
load("working_data/listEnrich/permutTable.1000times.block_fc2passModule.Parikshak.RData")
Parikshak=permTable
load("working_data/listEnrich/permutTable.1000times.block_fc2passModule.Voinagu.RData")
voin=permTable
...
for(i in 2:ncol(MEs$eigengenes)) {
  ...
  cairo_pdf(file=paste0("working_data//figures/moduleAnnot.blocks/all_details/", moduleNumber, moduleColor, ".pdf"), width=20, height=16,family = "Arial")
  ...
  #Hub gene network
  hubGenes = moduleGenes[order(kMEtable[moduleGenes,i], decreasing = T)[1:40]]
  ...
  g5=ggplot(ctdat)+
    geom_col(aes(CellType,sd_from_mean),width = 0.6)+
  ...
  grid.arrange(grobs=list(g1,g3,g4,g5,g2), layout_matrix=rbind(c(1,4,4,5,5,5),c(2,2,2,3,3,3),c(2,2,2,3,3,3)),
