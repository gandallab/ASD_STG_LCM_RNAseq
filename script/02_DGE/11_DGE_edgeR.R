...
design=model.matrix(~Diagnosis + Sex + Age + seqPC1 + seqPC2 + seqPC3,data = colData(se_neuron)) # removing RIN as a covariate increases the number of significant genes
y <- estimateDisp(y, design,robust = TRUE)
fit <- glmQLFit(y, design, robust=TRUE)
coef(fit)
qlf <- glmQLFTest(fit, coef=2)
