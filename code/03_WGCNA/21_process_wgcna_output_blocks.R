...
graph2ppt(file="working_data/figures/blocks.wgcna.pptx",width=8.5,height=6.5)

# process wgcna
rm(list=ls());gc()
options(stringsAsFactors = F)
load("working_data/wgcna/blocks.bicor/blocks.bicor.recut.RData")
load("working_data/wgcna/voom.forWGCNA.input.blocks.RData")
...
graph2ppt(file="working_data/figures/blocks.wgcna.pptx",width=8.5,height=6.5,append=TRUE)
...
print(p)
graph2ppt(file="working_data/figures/blocks.wgcna.pptx",width=7.8,height=2.7,append=TRUE)

write.table(modTrait,file = "working_data/wgcna/blocks.bicor/modTrait.blocks.bicor.txt",
            quote = F,
            sep="\t",
            row.names = F)
...
for(i in 2:length(unique(modules))){
  ...
  write.table(go,file=paste("working_data/wgcna/blocks.bicor/go/go.M",moduleNumber,"_",moduleColor,".txt",sep=""),quote=F,sep="\t",row.names=F)
}
